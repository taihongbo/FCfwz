...
namespace FCfwz
{
    public partial class Form1 : Form
    {
        ...
        public List<s_m_total> Get_m1_total()
        {
            ...
            if (DateTime.Parse(cEnd) >= DateTime.Parse("2019-03-15"))
            {
                this.dateTimePicker2.Value = DateTime.Parse("2019-03-15");
            }
        ...
        private void UpdateDefault(SqlConnection conn)
        {
            ...
            string sql = "";
            int nRet = 0;
            sql = " Update H4_收款记录 Set 科码='',科名='' Where 日期>='" + a + "' And 日期<='" + b + "' And ( 科码='' or 科名='') ";
            nRet = ExecuteNonQuery(conn, sql);
            sql = " Update H4_收款记录 Set  H4_收款记录.收入编码= H0_收费项目.收入编码 ,H4_收款记录.收入类型= H0_收费项目.收入名称 From H0_收费项目 Where H0_收费项目.项目编码=H4_收款记录.项目编码 And  H4_收款记录.日期>='" + a + "' And H4_收款记录.日期<='" + b + "' And ( H4_收款记录.收入编码= '' Or H4_收款记录.收入类型='' )";
            nRet = ExecuteNonQuery(conn, sql);
